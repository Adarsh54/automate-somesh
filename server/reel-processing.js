...
export async function prepareAudio(asset,{sign=signedAudio,store=put}={}){
 ...
 if(!info.streams.some(s=>s.codec_type==='audio') || !Number.isFinite(duration) || duration<=0 || duration>1200.1)throw Object.assign(new Error('Reel tracks must be audio files up to 20 minutes long.'),{status:400});
 const common=[...input,'-v','error','-nostdin','-threads','1','-i',url,'-map','0:a:0','-vn','-t','1200'];
