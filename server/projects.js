...
const stateSchema=z.object({
  ...
  silenceGap:z.number().min(0).max(10),thresholdDb:z.number().min(-90).max(-10),matchThreshold:z.number().min(0).max(1),
...
export function createProjectRepository(query) {
  return {
    async list(userId) {
      return query`SELECT id,title,revision,updated_at FROM projects WHERE user_id=${userId} ORDER BY updated_at DESC LIMIT 100`;
    },
    async get(userId,id) {
