...
export function collectionRow({title, detail, icon, actions, metadata=''}) {
  return `<article class="collection-row"><span class="collection-icon" aria-hidden="true">${icon}</span><div class="collection-info"><strong>${title}</strong><small>${detail}</small></div>${metadata}<div class="collection-actions">${actions}</div></article>`;
}
