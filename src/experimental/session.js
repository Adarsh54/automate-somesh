...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:v} of batchSchema.parse(commands)){
  ...
  const effectChains=[session.masterEffects,...session.tracks.map(t=>t.effects)],automationChains=[session.masterAutomation,...session.tracks.map(t=>t.automation)];
  ...
  switch(op){
   ...
   case 'send.set':{need(t,'Track');pick(v,['busId','gainDb','tap']);const send=t.sends.find(s=>s.busId===v.busId);if(send){if(v.gainDb!==undefined)send.gainDb=v.gainDb;if(v.tap!==undefined)send.tap=v.tap;}else t.sends.push({busId:v.busId,gainDb:v.gainDb,tap:v.tap===undefined?'postPan':v.tap});break;}
