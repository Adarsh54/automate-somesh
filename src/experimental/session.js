...
export const sessionSchema=z.object({version:z.literal(1),id:ident,title:z.string().max(200),revision:z.number().int().nonnegative(),tempo:z.number().min(20).max(300),meter:z.number().int().min(1).max(16),masterDb:db,masterEffects:z.array(effectSchema).max(16).default([]),loopEnabled:z.boolean().default(false),loopStart:time.default(0),loopEnd:time.default(4),frameRate:z.number().refine(value=>frameRates.includes(value),'Unsupported frame rate.').default(24),tracks:z.array(track).max(128),markers:z.array(z.object({id:ident,name:z.string().max(200),time})).max(1000)});
export const newSession=()=>({version:1,id:crypto.randomUUID(),title:'Untitled session',revision:0,tempo:120,meter:4,masterDb:0,masterEffects:[],tracks:[],markers:[]});
...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 const ids=[session.id,...session.masterEffects.map(e=>e.id),...session.tracks.flatMap(t=>[t.id,...t.effects.map(e=>e.id),...t.automation.map(p=>p.id),...t.regions.flatMap(r=>[r.id,...r.notes.map(n=>n.id),...r.events.map(e=>e.id)])]),...session.markers.map(m=>m.id)];if(new Set(ids).size!==ids.length)throw Error('IDs must be unique.');
