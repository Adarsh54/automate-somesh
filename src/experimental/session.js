...
const region=z.object({id:ident,name:z.string().max(200),assetId:ident.nullable(),mute:z.boolean().default(false),start:time,offset:time,duration:z.number().positive().max(86400),gainDb:db,fadeIn:time,fadeOut:time,reverse:z.boolean(),notes:z.array(note).max(20000),events:z.array(midiEventSchema).max(20000).default([])});
const track=z.object({id:ident,name:z.string().max(200),kind:z.enum(['audio','midi','video','bus']),gainDb:db,pan:z.number().min(-1).max(1),mute:z.boolean(),solo:z.boolean(),instrument:z.enum(['sine','triangle','square','sawtooth','drumKit','sampler']),sampleAssetId:ident.nullable().default(null),sampleRoot:z.number().int().min(0).max(127).default(60),sampleLoop:z.boolean().default(false),sampleLoopStart:time.default(0),sampleLoopEnd:time.nullable().default(null),regions:z.array(region).max(1000),output:ident.nullable().default(null),sends:z.array(z.object({busId:ident,gainDb:db,tap:z.enum(['preFader','postFader','postPan']).default('postPan'),automation:z.array(automationSchema.refine(p=>p.parameter==='gainDb','Send automation supports gain only.')).max(2000).default([])})).max(16).default([]),effects:z.array(effectSchema).max(16).default([]),automation:z.array(automationSchema).max(2000).default([])}).superRefine((t,ctx)=>{if(t.sampleLoopEnd!==null&&t.sampleLoopEnd<=t.sampleLoopStart)ctx.addIssue({code:'custom',message:'Sampler loop end must follow its start.'});});
...
export const operations=['midi.import','session.set','track.add','track.duplicate','track.move','track.set','track.delete','send.set','send.delete','send.automation.point','send.automation.clear','region.add','region.extractAudio','region.trim','region.set','region.move','region.delete','region.split','region.duplicate','region.repeat','event.ramp','event.add','event.set','event.delete','note.add','note.set','note.delete','notes.scale','notes.chord','notes.velocity','notes.move','notes.resize','notes.delete','notes.duplicate','notes.quantize','notes.humanize','notes.transpose','marker.add','marker.set','marker.delete','effect.add','effect.set','effect.delete','effect.move','automation.point','automation.set','automation.delete','automation.clear'];
...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:v} of batchSchema.parse(commands)){
  ...
  switch(op){
   ...
   case 'track.add':session.tracks.push(track.parse({id:crypto.randomUUID(),name:'New track',kind:'audio',gainDb:0,pan:0,mute:false,solo:false,instrument:'triangle',regions:[],...pick(v,['id','name','kind','instrument','sampleAssetId','sampleRoot','sampleLoop','sampleLoopStart','sampleLoopEnd'])}));break;
   ...
   case 'track.set':Object.assign(need(t,'Track'),pick(v,['name','gainDb','pan','mute','solo','instrument','sampleAssetId','sampleRoot','sampleLoop','sampleLoopStart','sampleLoopEnd','output']));break;
   ...
   case 'region.set':Object.assign(need(r,'Region'),pick(v,['name','start','offset','duration','gainDb','fadeIn','fadeOut','reverse','mute']));break;
