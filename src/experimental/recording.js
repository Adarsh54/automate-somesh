...
export async function startRecording({signal,session=null,position=0,onProgress=()=>{},onLimit=()=>{}}={}){
 ...
 try{
  ...
  const startFrame=Math.ceil((context.currentTime+.025)*context.sampleRate),baseTime=startFrame/context.sampleRate;
  ...
  source.connect(node).connect(context.destination);if(session?.metronomeRecordEnabled)click=scheduleMetronome(context,{...session,metronomeEnabled:true},{position,baseTime,duration:600});timer=setInterval(()=>onProgress({seconds:frames/context.sampleRate,peak}),100);
