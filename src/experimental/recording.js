...
export async function startRecording({signal,onProgress=()=>{},onLimit=()=>{}}={}){
 ...
 let stream,context,node,source,timer,closed=false,resolveStop;const chunks=[];let frames=0,channels=0,peak=0,limitReached=false;
 const cleanup=()=>{clearInterval(timer);stream?.getTracks().forEach(t=>t.stop());source?.disconnect();node?.disconnect();if(context&&context.state!=='closed')context.close().catch(()=>{});};
 ...
 try{
  ...
  node=new AudioWorkletNode(context,'cuestamp-capture');source=context.createMediaStreamSource(stream);
