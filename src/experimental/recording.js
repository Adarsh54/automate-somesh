...
export async function startRecording({signal,session=null,buffers=new Map(),position=0,onProgress=()=>{},onLimit=()=>{}}={}){
 ...
 let stream,context,node,source,timer,click,backing,closed=false,resolveStop;const chunks=[];let frames=0,channels=0,peak=0,limitReached=false;
 const cleanup=()=>{backing?.stop();click?.stop();clearInterval(timer);stream?.getTracks().forEach(t=>t.stop());source?.disconnect();node?.disconnect();if(context&&context.state!=='closed')context.close().catch(()=>{});};
 ...
 try{
  ...
  source.connect(node).connect(context.destination);click=scheduleRecordingClick(context,session,position,timing);backing=scheduleRecordingPlayback(context,session,buffers,position,captureTime);timer=setInterval(()=>onProgress({seconds:frames/context.sampleRate,peak,countInRemaining:Math.max(0,captureTime-context.currentTime)}),100);
