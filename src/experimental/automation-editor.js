...
export function automationView(track,esc,{gainOnly=false}={}){
...
export function bindAutomation(root,{track,execute,guard,duration,automationParameter='gainDb',onAutomationParameter=()=>{},
 ...
 function draw(){
  const key=parameter.value,min=key==='gainDb'?-96:-1,max=key==='gainDb'?12:1;
  ...
  const coords=event=>{
   ...
   return {time:round(clamp(((event.clientX-rect.left)/rect.width*600-8)/584,0,1)*end),value:round(clamp(max-((event.clientY-rect.top)/rect.height*160-8)/144*(max-min),min,max))};
  };
  ...
  const edit=(id,values)=>execute([{op:'automation.set',target:id,values}],'Moved automation point');
  graph.querySelectorAll('[data-auto-point]').forEach(handle=>{
   ...
   handle.onkeydown=guard(e=>{
    ...
    const step=e.shiftKey?10:1,values=e.key==='ArrowLeft'||e.key==='ArrowRight'
     ?{time:round(clamp(original.time+(e.key==='ArrowRight'?1:-1)*.1*step,0,86400))}
     :{value:round(clamp(original.value+(e.key==='ArrowUp'?1:-1)*(key==='gainDb'?.5:.05)*step,min,max))};
    edit(original.id,values);
    host.querySelector(`[data-auto-point="${original.id}"]`)?.focus();
   });
   handle.onpointerdown=e=>{
    if(e.button!==0)return;e.stopPropagation();handle.focus();handle.setPointerCapture(e.pointerId);
    ...
    handle.onpointerup=guard(event=>{event.stopPropagation();reset();handle.releasePointerCapture(event.pointerId);if(moved&&values)edit(original.id,values);});
   };
  });
  pointsRoot.innerHTML=points.map(p=>`<div><form data-auto-edit="${p.id}"><label>Time · seconds<input name="time" type="number" min="0" max="86400" step="any" value="${p.time}" required></label><label>${key==='gainDb'?'Volume · dB':'Pan'}<input name="value" type="number" min="${min}" max="${max}" step="any" value="${p.value}" required></label><button type="submit">Update point</button><button type="button" data-auto-remove="${p.id}" aria-label="Remove automation point at ${p.time} seconds">Remove</button></form></div>`).join('');
  pointsRoot.querySelectorAll('[data-auto-edit]').forEach(editor=>editor.onsubmit=guard(e=>{e.preventDefault();edit(editor.dataset.autoEdit,{time:Number(editor.elements.time.value),value:Number(editor.elements.value.value)});}));
  ...
  form.elements.value.min=min;form.elements.value.max=max;
