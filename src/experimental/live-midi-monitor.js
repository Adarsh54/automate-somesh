...
export function createLiveMidiMonitor(context, {destination=context.destination,maxVoices=64}={}) {
 ...
 function push(data){
  ...
  if(kind===0x90&&b){
   ...
   osc.type='triangle';osc.frequency.value=440*2**((a-69)/12);osc.detune.value=c.bend*200;
   gain.gain.setValueAtTime(0,now);gain.gain.linearRampToValueAtTime(b/127*.18,now+.008);
   osc.connect(gain);gain.connect(c.gain);const v={osc,gain,channel:id,pitch:a,held:true,released:false};voices.add(v);osc.onended=()=>destroy(v);osc.start();
  }else if(kind===0x80||(kind===0x90&&!b)){
   const v=matching().find(v=>v.pitch===a&&v.held);if(v){v.held=false;if(!c.sustain)release(v);}
  }else if(kind===0xe0){const bend=(a|(b<<7))-8192;c.bend=bend/(bend<0?8192:8191);for(const v of matching())v.osc.detune.setValueAtTime(c.bend*200,context.currentTime);
  }else if(kind===0xb0){
   ...
   if(a===64){c.sustain=b>=64;if(!c.sustain)for(const v of matching())if(!v.held)release(v);}
   if(a===120)for(const v of matching())release(v,true);
   if(a===123)for(const v of matching()){v.held=false;if(!c.sustain)release(v);}
   if(a===121){c.expression=1;c.bend=0;c.sustain=false;update(c);for(const v of matching()){v.osc.detune.setValueAtTime(0,context.currentTime);if(!v.held)release(v);}}
  }
 }
