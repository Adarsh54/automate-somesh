...
export function metronomeView(session){return `<form class="daw-cycle" data-metronome-form><label><input name="enabled" type="checkbox" ${session.metronomeEnabled?'checked':''}> Metronome during playback</label><label><input name="recordEnabled" type="checkbox" ${session.metronomeRecordEnabled?'checked':''}> During recording</label><label>Beats per bar · quarter notes<input name="meter" type="number" min="1" max="16" step="1" value="${session.meter}"></label><label>Click level · dB<input name="level" type="number" min="-60" max="0" step="1" value="${session.metronomeDb??-18}"></label><button type="submit">Apply metronome</button><small>Excluded from captured audio and exports. Use headphones while recording to prevent microphone bleed.</small></form>`;}
export function bindMetronome(root,{execute,guard}){const form=root.querySelector('[data-metronome-form]');form.onsubmit=guard(e=>{e.preventDefault();execute([{op:'session.set',values:{metronomeEnabled:form.elements.enabled.checked,metronomeRecordEnabled:form.elements.recordEnabled.checked,metronomeDb:Number(form.elements.level.value),meter:Number(form.elements.meter.value)}}],'Updated metronome');});}
