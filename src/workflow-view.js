...
export function workflowView(state, workflow, { esc, field, select }) {
  ...
  const uploadedTracks = mode === "offset" || !tracks.length ? "" : `<div class="uploaded-tracks" aria-label="Uploaded audio tracks">${tracks.map(t => `<div class="uploaded-track"><strong>${esc(t.filename)} <small>${time(t.duration)}</small></strong>${removeTrack(t)}</div>`).join("")}</div>`;
