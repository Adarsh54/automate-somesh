export function duplicateProject(project, projects = []) {
 ...
 const base = (data.type === 'daw' ? data.session.title : data.type === 'reel' ? data.title : data.production.title)?.trim() || 'Untitled project';
 ...
 if (data.type === 'daw') data.session.title = title;
 else if (data.type === 'reel') data.title = title;
