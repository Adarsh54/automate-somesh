...
export function createReelWorkspace({account,audioLibrary,esc,onChange}){
 ...
 async function save(){
  ...
  try{
   ...
   active={id:project.id,revision:project.revision};data=snapshot;dirty=false;persist();notice='Reel draft saved to Projects.';reelsLoaded=false;
 ...
 function reelsView(){return account.user?`<section class="saved-reels"><div class="section-title"><h2>Your reels</h2>${collectionCreateButton({label:"Create reel",attributes:"id=\"new-saved-reel\"",disabled:busy})}</div><div class="collection-list">${savedReels.map(p=>collectionRow({title:`<button class="project-title-link" data-edit-reel="${esc(p.id)}">${esc(p.title)}</button>`,detail:`${p.published?'Published':'Draft'}<span class="project-dates">${projectDates(p,esc)}</span>`,icon:'▷',actions:`<button data-edit-reel="${esc(p.id)}" ${busy?'disabled':''}>Edit</button>${p.published?`<button data-stop-reel="${esc(p.id)}" ${busy?'disabled':''}>Stop sharing</button>`:''}`})).join('')||'<p class="muted">Saved reels appear here. Use Create project to start another reel.</p>'}</div></section>`:'';}
 async function openReel(id){
  if(dirty&&!await confirmDialog({title:'Save before switching reels?',message:'Save your changes or continue without saving.',confirmLabel:'Save and continue',secondaryLabel:'Continue without saving',onConfirm:save}))return;
  const {project}=await libraryRequest('/api/projects?id='+encodeURIComponent(id));reset();data=project.data;active={id:project.id,revision:project.revision};dirty=false;persist();onChange();window.scrollTo({top:0,behavior:'smooth'});
 }
 function bind(){
  document.querySelector("#new-saved-reel")?.addEventListener("click",run(async()=>{if(dirty&&!await confirmDialog({title:"Save before creating a reel?",message:"Save your changes or continue without saving.",confirmLabel:"Save and continue",secondaryLabel:"Continue without saving",onConfirm:save}))return;reset();data=blank();active=null;dirty=false;persist();onChange();window.scrollTo({top:0,behavior:"smooth"});}));
  document.querySelectorAll("[data-edit-reel]").forEach(button=>button.onclick=run(()=>openReel(button.dataset.editReel)));
