import {uploadAudioFile} from './audio-upload.js';
...
export function createAudioLibrary({account,esc,onChange,request=libraryRequest,uploadFile=upload}) {
 ...
 let local=[],remote=[],loaded=false,loading=false,error='',progress='',uploading=false,loadPromise=null,uploadController=null;
 ...
 async function addFile(file){
  ...
  if(account.user && !item.assetId){
   ...
   if(!item.uploaded){await uploadAudioFile(uploadFile,asset.pathname,file,{access:'private',handleUploadUrl:'/api/media',clientPayload:asset.id,contentType:asset.contentType,multipart:true},{onController:controller=>{uploadController=controller;onChange();},onProgress:({percentage})=>{const next=`Uploading ${file.name} — ${Math.floor(percentage)}%`;if(progress!==next){progress=next;onChange();}}});item.uploaded=true;await localOperation('readwrite',store=>store.put(item));}
 ...
 return {cancelUpload,uploadStatus:()=>progress,canCancelUpload:()=>Boolean(uploadController),load,add,addMany,fileFor,entries,view,bind,pick,isBusy:()=>uploading};
}
