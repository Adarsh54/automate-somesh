export function showDownloadDialog({title, saved, download}) {
  ...
  dialog.innerHTML=`<div class="completion-mark" aria-hidden="true">✓</div><h2 id="download-heading">Your cue sheet is ready</h2><p data-title></p><p class="muted">${saved?'Completed and saved to Projects.':'Completed on this device. Sign in to save it to Projects.'}</p><label for="download-format">Download format</label><select id="download-format"><option value="xlsx">Excel (.xlsx) — BMI template</option><option value="csv">CSV (.csv) — spreadsheet data</option><option value="pdf">PDF (.pdf) — printable cue sheet</option></select><p class="muted" data-format-help>Uses BMI’s Excel template with a separate frame timings worksheet.</p><p role="status" data-download-status></p><div class="button-row"><button class="primary" data-download>↓ Download cue sheet</button>${saved?'<button data-projects>View Projects</button>':''}<button data-close>Close</button></div>`;
  ...
  select.onchange=()=>{dialog.querySelector('[data-format-help]').textContent=select.value==='xlsx'?'Uses BMI’s Excel template with a separate frame timings worksheet.':select.value==='csv'?'One row per cue contributor, with exact timecodes and production details.':'A printable summary of production details, cue timings and contributor credits.';};
  button.onclick=async()=>{
    ...
    try {await download(select.value);status.textContent='Downloaded. You can choose another format.';}
