...
function render() {
  ...
  document.querySelector("#sidebar-nav")?.insertAdjacentHTML("beforeend", `<a class="nav ${steps.some(([key])=>key===tab)?'active':''}" href="#/workspace" aria-label="New Cue Workspace" title="New Cue Workspace" ${steps.some(([key])=>key===tab)?'aria-current="page"':''}>${sidebarIcon('library')}<span class="nav-label">New Cue Workspace</span></a><a class="nav ${tab==='reel'?'active':''}" href="#/reels/new" aria-label="New Reel" title="New Reel" ${tab==='reel'?'aria-current="page"':''}>${sidebarIcon('reel')}<span class="nav-label">New Reel</span></a><a class="nav ${tab==='audio'?'active':''}" href="#/audio" aria-label="Audio Files" title="Audio Files" ${tab==='audio'?'aria-current="page"':''}>${sidebarIcon('audio')}<span class="nav-label">Audio Files</span></a>`);
