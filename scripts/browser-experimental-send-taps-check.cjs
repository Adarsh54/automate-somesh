...
(async()=>{
 ...
 try {
  ...
  await curve.locator('[name=value]').fill('-24');await curve.getByRole('button',{name:'Add point',exact:true}).click();
  assert.equal(await curve.getAttribute('open'),'');
  await curve.locator('[name=time]').fill('1');await curve.locator('[name=value]').fill('0');
