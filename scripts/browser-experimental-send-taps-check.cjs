...
(async()=>{
 ...
 try {
  ...
  const metrics=await page.evaluate(async()=>{
   ...
   async function render(tap,{mute=false,bus=false,filtered=false,automated=false}={}){
    ...
    scheduleSession(ctx,s,new Map([['file',buffer]]),0,{baseTime:0});const audio=await ctx.startRendering();
  ...
  assert.deepEqual(errors,[]);console.log('PASS send position UI, undo/persistence, PCM fader/pan independence, automation, post-insert filtering and source/bus mute.',metrics);
