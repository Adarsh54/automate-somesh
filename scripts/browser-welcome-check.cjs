...
(async()=>{
 ...
 await page.getByRole('heading',{name:'Welcome to Cuebook'}).waitFor();
 ...
 await page.screenshot({path:'/tmp/cuebook-welcome-desktop.png',fullPage:true});
 await page.setViewportSize({width:390,height:844});
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
 await page.screenshot({path:'/tmp/cuebook-welcome-mobile.png',fullPage:true});
 ...
 await page.goto((process.env.CUEBOOK_URL||'http://127.0.0.1:5181/')+'?authError=1');
