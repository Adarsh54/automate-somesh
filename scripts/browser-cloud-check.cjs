...
(async()=>{
 ...
 await page.goto(process.env.CUESTAMP_URL||'http://127.0.0.1:5176/');
 ...
 await page.evaluate(()=>{
  const legacy=JSON.parse(localStorage.getItem('cuestamp-user:alice:draft'));
  legacy.production.title='Imported browser film';localStorage.setItem('cuestamp-v1',JSON.stringify(legacy));
 });
 await page.locator('#cloud-import').click();
 await page.waitForFunction(()=>JSON.parse(localStorage.getItem('cuestamp-user:bob:draft')||'null')?.production.title==='Imported browser film');
 await page.locator('#sidebar-nav [data-tab="production"]').click();
 assert.equal(await page.locator('#production [data-field="title"]').inputValue(),'Imported browser film');
 assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('cuestamp-v1')).production.title),'Imported browser film');
