...
(async()=>{
 ...
 try{
  ...
  if(!process.env.CUESTAMP_REEL_URL){
   ...
   await page.locator('.reel-player').waitFor();await page.getByRole('button',{name:'Play',exact:true}).click();await page.waitForFunction(()=>document.querySelector('.reel-now time')?.textContent.startsWith('0:01'));
