...
(async()=>{
 ...
 await page.locator('[data-mode="offset"]').click();await page.locator('[data-upload]').first().setInputFiles(`${process.env.FIXTURES||'/tmp/cuestamp-fixtures'}/score.wav`);await ready();
 ...
 await nav('Find your cues');await page.locator('[data-track]').click();await page.locator('[data-reattach]').setInputFiles(`${process.env.FIXTURES||'/tmp/cuestamp-fixtures'}/score.wav`);await ready();
 ...
 await page.locator('[data-cue]').first().screenshot({path:'/tmp/cuestamp-cue-details.png'});
 await page.setViewportSize({width:390,height:844});
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
 await page.locator('[data-cue]').first().locator('[data-field="last"]').scrollIntoViewIfNeeded();
 await page.screenshot({path:'/tmp/cuestamp-cue-mobile.png'});
 ...
 const download=page.waitForEvent('download');await page.locator('#export').click();await (await download).saveAs('/tmp/cuestamp-cue-details.xlsx');
