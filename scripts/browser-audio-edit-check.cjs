...
(async()=>{
 ...
 try{
  ...
  await page.route('**/api/reels*',r=>{
   ...
   const body=r.request().postDataJSON();if(new URL(r.request().url()).searchParams.get('action')==='prepare')return r.fulfill({json:{track:{duration:10,peaks:Array.from({length:160},(_,i)=>.2+.7*Math.abs(Math.sin(i/7)))}}});requests.push(body);
  ...
  const dialog=page.locator('.audio-edit-dialog');await dialog.locator('form').waitFor();assert.equal(await dialog.locator('audio[controls]').count(),0);assert.equal(await dialog.locator('audio').isVisible(),false);await dialog.locator('[data-length]').filter({hasText:'selected'}).waitFor();const box=await dialog.locator('.edit-wave-surface').boundingBox();await page.mouse.move(box.x+box.width*.2,box.y+80);await page.mouse.down();await page.mouse.move(box.x+box.width*.7,box.y+80);await page.mouse.up();assert.ok(Math.abs(Number(await dialog.locator('[name=start]').inputValue())-2)<.1);assert.ok(Math.abs(Number(await dialog.locator('[name=end]').inputValue())-7)<.1);await dialog.locator('[data-handle=start]').focus();await page.keyboard.press('ArrowRight');assert.ok(Number(await dialog.locator('[name=start]').inputValue())>2);await dialog.locator('[name=end]').fill('3');await dialog.locator('[name=start]').fill('1');await dialog.locator('[name=fadeIn]').fill('.5');await dialog.locator('[name=fadeOut]').fill('.5');await dialog.locator('[name=normalize]').check();await dialog.locator('[name=targetPeakDb]').fill('-6');
  ...
  assert.equal(await page.locator('[data-library-edit]').count(),2);assert.equal(requests[0].mode,'copy');assert.equal(requests[0].edit.targetPeakDb,-6);
