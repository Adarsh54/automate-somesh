...
(async()=>{
 ...
 try {
  ...
  await page.getByRole('button',{name:'Master effects · 0',exact:true}).click();
  ...
  await page.reload();await page.getByRole('button',{name:'Master effects · 2',exact:true}).click();
  ...
  const metrics=await page.evaluate(async()=>{
   ...
   async function render(effect=null,{gain=0,opposite=false,impulse=false,bypass=false}={}){
    const initial=newSession(),commands=[{op:'session.set',values:{masterDb:gain}}];
    ...
    const playback=scheduleSession(ctx,session,buffers,0,{baseTime:0});const output=await ctx.startRendering();playback.stop();return {data:output.getChannelData(0),duration:output.duration};
   }
   ...
   return {dry:rms(dry.data),filtered:rms(filtered.data),bypass:rms(bypass.data),compressed:rms(compressed.data),quiet:rms(quiet.data),cancelled:rms(cancelled.data),tail:rms(reverb.data,1.1,1.5),duration:reverb.duration};
  });
  ...
  await page.locator('.daw-master-effects').scrollIntoViewIfNeeded();await page.screenshot({path:'/tmp/cuestamp-master-effects.png'});
  assert.deepEqual(errors,[]);console.log('PASS master chain UI, persistence, bypass/reorder/undo, summed PCM processing, post-effect master gain and reverb tails.',metrics);
