...
Object.assign(process.env,{WORKOS_API_KEY:'test',WORKOS_CLIENT_ID:'test',SESSION_SECRET:'x'.repeat(40),APP_URL:'https://cuebook.test',DATABASE_URL:'postgresql://unused'});
...
test('private media ownership, completion and project references are enforced by Postgres',async()=>{
 ...
 try {
  ...
  const overwrite=response();await owner({method:'POST',url:'/api/media',headers:{origin:'https://cuebook.test','content-type':'application/json'},body:{type:'blob.generate-client-token'}},overwrite);assert.equal(overwrite.code,400);
