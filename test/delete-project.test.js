...
test('deleting a reel requires its owner and current revision, cascades sharing, and preserves library audio',async()=>{
 ...
 try{
  ...
  await assert.rejects(repo.deleteProject('bob',{id,revision:1}),{status:409});
  await assert.rejects(repo.deleteProject('alice',{id,revision:2}),{status:409});
  await assert.rejects(repo.deleteProject('alice',{id,revision:0}),{status:400});
  assert.equal((await repo.get('alice',id)).revision,1);
  assert.deepEqual(await repo.deleteProject('alice',{id,revision:1}),{id});
...
test('deleting a cue sheet requires its owner and current revision, and leaves other projects untouched',async()=>{
 ...
 try{
  ...
  const cueId=crypto.randomUUID(),otherId=crypto.randomUUID();
  await db.query('INSERT INTO projects(id,user_id,title,data) VALUES ($1,$2,$3,$4)',[cueId,'alice','Cue','{"type":"cue"}']);
  await db.query('INSERT INTO projects(id,user_id,title,data) VALUES ($1,$2,$3,$4)',[otherId,'alice','Keep me','{"type":"cue"}']);
  await assert.rejects(repo.deleteProject('bob',{id:cueId,revision:1}),{status:409});
  await assert.rejects(repo.deleteProject('alice',{id:cueId,revision:2}),{status:409});
  assert.deepEqual(await repo.deleteProject('alice',{id:cueId,revision:1}),{id:cueId});
  await assert.rejects(repo.get('alice',cueId),{status:404});
  assert.equal((await repo.get('alice',otherId)).title,'Keep me');
 }finally{await db.close();}
});
