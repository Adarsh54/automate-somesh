...
test('publishing snapshots owned prepared audio; revisions, leases, permissions and revocation are enforced',async()=>{
 const db=new PGlite();
 try{
  for(const file of ['001_users_projects.sql','002_media_assets.sql','007_audio_edits.sql','006_reels.sql','007_reel_analytics.sql','008_reel_listen_events.sql','009_reel_share_links.sql','010_folders.sql'])await db.exec(await readFile(new URL('../migrations/'+file,import.meta.url),'utf8'));
