...
test('hung audio enumeration times out and can be retried',async()=>{let hung=true;const inputs=createAudioInputs({timeoutMs:5,mediaDevices:{enumerateDevices:()=>hung?new Promise(()=>{}):Promise.resolve([{kind:'audioinput',deviceId:'mic',label:'Ready'}])}});inputs.activate();await new Promise(r=>setTimeout(r,15));assert.match(inputs.view(String),/discovery timed out/);assert.doesNotMatch(inputs.view(String),/data-audio-input-refresh disabled/);hung=false;await inputs.refresh();assert.match(inputs.view(String),/Ready/);assert.doesNotMatch(inputs.view(String),/timed out/);inputs.dispose();});
